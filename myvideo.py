    ...
    while(True):
        ...
        if ref == True:
            if False:    
                ...
                # 进行检测
                frame, ret = yolo.detect_image(frame, dstpath, fno)
                ...
                ref, frame = retinaface.detect_image(frame, dstpath, fno)
                # RGBtoBGR满足opencv显示格式
                #frame = cv2.cvtColor(frame,cv2.COLOR_RGB2BGR)
...
resultlst = []
# 传入空的list接收文件名
abs_contents, filename = show_files("/home/wsun/srcvideo/sdv1/20170822", [], [])
